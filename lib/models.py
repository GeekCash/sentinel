...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, geekcashd):
        golist = geekcashd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_geekcashd(geekcashd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_geekcashd(self, geekcashd, rec):
        import decimal
        import geekcashlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from geekcashd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from geekcashd, with every run
        ...
        # get/create, then sync payment amounts, etc. from geekcashd - geekcashd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(geekcashd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from geekcashd! %s" % e)
            govobj.vote_delete(geekcashd)
            return (govobj, None)
    ...
    def vote_delete(self, geekcashd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(geekcashd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, geekcashd, signal, outcome):
        import geekcashlib
        ...
        output = geekcashd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = geekcashlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(geekcashd, signal)

    def sync_network_vote(self, geekcashd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = geekcashd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = geekcashD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 geekcash addr, non-multisig
            if not geekcashlib.is_valid_geekcash_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid GeekCash address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # GeekCash Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (geekcashlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = geekcashD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not geekcashlib.is_valid_geekcash_address(addr, config.network):
    ...
    def hash(self):
        import geekcashlib
        return geekcashlib.hashit(self.serialise())
