...
class GovernanceClass(object):
    ...
    def vote(self, geekcashd, signal, outcome):
        return self.go.vote(geekcashd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, geekcashd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(geekcashd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(geekcashd, models.VoteSignals.valid, models.VoteOutcomes.no)

    def get_submit_command(self):
        import geekcashlib
        obj_data = geekcashlib.SHIM_serialise_for_geekcashd(self.serialise())
    ...
    def geekcashd_serialise(self):
        import geekcashlib
        return geekcashlib.SHIM_serialise_for_geekcashd(self.serialise())
