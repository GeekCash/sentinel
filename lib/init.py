...
def has_geekcash_conf():
    import config
    import io

    valid_geekcash_conf = False

    # ensure geekcash_conf exists & readable
    #
    # if not, print a message stating that GeekCash Core must be installed and
    # configured, including JSONRPC access in geekcash.conf
    try:
        f = io.open(config.geekcash_conf)
        valid_geekcash_conf = True
    except IOError as e:
        print(e)

    return valid_geekcash_conf
...
def main():
    ...
    if not has_geekcash_conf():
        print("geekcashCore must be installed and configured, including JSONRPC access in geekcash.conf")
        sys.exit(1)
