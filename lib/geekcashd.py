...
class GeekCashDeamon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some geekcashd methods
    ...
    @classmethod
    def from_geekcash_conf(self, geekcash_dot_conf):
        from geekcash_config import GeekCashConfig
        config_text = GeekCashConfig.slurp_config_file(geekcash_dot_conf)
        creds = GeekCashConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = geekcashlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = geekcashlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = geekcashlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_geekcashd_proto_version_with_sentinel_ping)
