...
def is_valid_geekcash_address(address, network='mainnet'):
    ...
    geekcash_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid GeekCash address
        return False

    if (address_version != geekcash_version):
        return False
...
def create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize):
    ...
    for proposal in proposals:
        ...
        data_size = len(sb_temp.geekcashd_serialise())
...
def SHIM_serialise_for_geekcashd(sentinel_hex):
    ...
    # superblock => "trigger" in geekcashd
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # geekcashd expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    geekcashd_hex = serialise(obj)
    return geekcashd_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('geekcash.conf')
